...
class App
{
	...
	Controller(){ // Controller
		this.num = 1 , this.x = 3 , this.y = 1 , this.sy = 1 , this.Ay = this.y , this.Ax = this.x;
		this.Create_Block();
	...
	Update(){ // Update
		...
		this.AfterArr.forEach( (v,v_pos)=>{
			...
			this.ctx.rect((this.Tetris_left+50) +(this.Tetris_row*this.rect),(this.Tetris_top+this.rect/2)+(200*v_pos), this.rect*2.5, this.rect*2.5 );
			this.ctx.stroke();
			this.ctx.closePath();
			v.arr.forEach( (y,y_pos) => {
				y.forEach( (x,x_pos)=>{
					if( x == "ㅁ"){
						...
						this.ctx.rect((this.Tetris_left+50)+(this.Tetris_row*this.rect)+(this.rect/1.7*x_pos), (this.Tetris_top+this.rect/2)+(this.rect/1.7*y_pos)+(200*v_pos), this.rect/1.7, this.rect/1.7 );
						this.ctx.fill();
						this.ctx.closePath();
					}
				} )
		...
		this.ctx.fillText(this.Score,(this.Tetris_left+50) +(this.Tetris_row*this.rect),(this.Tetris_top+this.rect/2)+(200*4));
		this.ctx.closePath();
		...
		if( this.end ){
			...
			this.ctx.fill();
			this.ctx.closePath();
			setTimeout( ()=>{ if(confirm("다시 하실 건가요?")) location.reload(); },500)			
		}
	...
	hold(temp){
		...
		if( this.holdNum != "a" )  this.order.unshift(this.holdNum);
	...
	KeyUp(x1=0,y1=0 ){ // KeyUp
		...
		for(let y = 0; y<this.column; y++){
			for(let x = 0; x<this.row; x++){
				if( this.temp[y][x] != "ㅁ") continue;
				if( this.obj_len == 1) y1 = -1 , x1 = 1
				else if( this.num == 1 &&  this.row == 3 && this.column == 2 || this.obj_len == 2 &&  this.row == 3 && this.column == 2 ) y1 =-1;
				else if( this.obj_len == 2 && this.num == 2 && this.row == 1 && this.column == 4) y1 = -1 , x1 = 2;
				else if( this.obj_len == 2 && this.num == 4 && this.row == 1 && this.column == 4) y1 = -1, x1 = 1;
				else if( this.num == 2 && this.row == 2 && this.column == 3) y1 = -1, x1 = 1;
				else if( this.num == 4 && this.row == 2 && this.column == 3) y1 = -1;
				if( this.Tetris[this.y+y+y1][this.x+x+x1].shape == "ㅇ" ){
					this.num--;
					if( this.num == 0) this.num = 4;
					this.x = this.Ax;
					this.y = this.Ay;
